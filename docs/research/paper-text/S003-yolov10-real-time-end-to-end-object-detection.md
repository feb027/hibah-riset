...
source_url: https://arxiv.org/pdf/2405.14458
source_file: docs/research/papers/S003-yolov10-real-time-end-to-end-object-detection.pdf
source_kind: pdf
extraction_note: downloaded PDF
---

# Extracted text: S003-yolov10-real-time-end-to-end-object-detection.pdf
## PDF metadata
- format: PDF 1.5
- creator: LaTeX with hyperref
- producer: pdfTeX-1.40.25
- creationDate: D:20241031002038Z
- modDate: D:20241031002038Z
...
example, our YOLOv10-S is 1.8× faster than RT-DETR-R18 under the simi-
...
m∗
o2m ≤u∗for j ∈Ωand to2o,i=u∗· mo2o,i
m∗
o2o =u∗
...
take r=1, by default, i.e., αo2o=αo2m and βo2o=βo2m. To verify the improved supervision alignment,
...
convolutions with stride of 2, achieving spatial downsampling (from H × W to H
2 × W
2 ) and channel
...
small objects, while also introducing significant I/O overhead and latency in high-resolution stages [8].
...
Table 1: Comparisons with state-of-the-arts. Latency is measured using official pre-trained models.
...
# Model
NMS-free. Efficiency. Accuracy. #Param.(M) FLOPs(G) APval(%) Latency(ms)
1
YOLOv10-S
11.2
28.6
...
Latency
1 base.
11.2/25.9 28.6/78.9 44.3/50.3 2.44/5.22
2 +cls.
9.9/23.2 23.5/67.7 44.2/50.2 2.39/5.07
3 +downs. 8.0/19.7 22.2/65.0 44.4/50.4 2.36/4.97
4 +block.
6.2/14.1 20.8/58.1 44.5/50.4 2.31/4.57
...
Tab. 2, under different αo2o and βo2o. As shown in Tab. 4, the proposed consistent matching metric,
...
[18] Golnaz Ghiasi, Yin Cui, Aravind Srinivas, Rui Qian, Tsung-Yi Lin, Ekin D Cubuk, Quoc V
...
[21] Jocher Glenn.
Yolov8.
https: // github. com/ ultralytics/ ultralytics/ tree/
main , 2023.
...
[53] Russell Stewart, Mykhaylo Andriluka, and Andrew Y Ng. End-to-end people detection in
...
[69] Haiyang Xu, Zhichao Zhou, Dongliang He, Fu Li, and Jingdong Wang. Vision transformer with
...
where I(·) is the indicator function. We denote the classification targets of the predictions in Ωas
...
decreases as ˆtn increases, indicating that n decreases and the ranking of i within Ωimproves. Due
to ˆtn ≤ˆt1, A thus achieves the minimum when ˆtn = ˆt1, i.e., i is the best positive sample in Ωwith
mo2m,i = m∗
...
we assume αo2o > 0 and βo2o > 0. We can obtain r1 =
αo2o
αo2m > 0 and r2 =
βo2o
βo2m > 0, and then
...
st ←argmins∈S R;
6
Θt+1 ←Replace(Θt, θcib, st) ; // Replace the block in Stage st of Θt with CIB θcib.
7
apt+1 ←AP(T(Θt+1));
8
if apt+1 ≥ap0 then
...
APval
50 and APval
75 at different IoU thresholds, as well as APval
small, APval
medium, and APval
large across
